...
TEST_CASE("v0 serialization - basic values", "[serialization]") {
    ...
    const auto expected_serialized = bmq::to_hex(pub_key.prefixed_hex()) +
    ...
    CHECK(bmq::to_hex(serialized.front()) == expected_serialized);
    ...
    CHECK(bmq::to_hex(batches[0]) == expected_serialized + expected_serialized);
