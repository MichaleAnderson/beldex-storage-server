...
bool verify_signature(
    ...
    if ((pubkey.type() == 5 || (pubkey.type() == 0 && !is_mainnet)) && pk_ed25519) {
        ...
        if (crypto_sign_ed25519_pk_to_curve25519(xpk.data(), pk) != 0
            ...
            BELDEX_LOG(debug, "Signature verification failed: ed -> x conversion did not match");
            return false;
        }
    }
    else
    ...
    bool verified = 0 == crypto_sign_verify_detached(
            ...
            pk);
    if (!verified)
        BELDEX_LOG(debug, "Signature verification failed");
    return verified;
}
...
Response RequestHandler::handle_wrong_swarm(const user_pubkey_t& pubKey) {
    ...
    BELDEX_LOG(trace, "Got client request to a wrong swarm");

    return {
        http::MISDIRECTED_REQUEST,
        swarm_to_json(master_node_.get_swarm(pubKey))};
}
...
struct swarm_response {
    ...
    std::function<void(beldex::Response)> cb;
};
...
void reply_or_fail(const std::shared_ptr<swarm_response>& res) {
    ...
    for (const auto& [mnode, reply] : res->result.items()) {
    ...
    for (auto& peer : peers) {
        mn.bmq_server()->request(
                ...
                [res, peer, cmd](bool success, auto parts) {
                    json peer_result;
                    if (!success)
                        BELDEX_LOG(warn, "Response timeout from {} for forwarded command {}",
...
static setup_recursive_request(MasterNode& mn, RPC& req, std::function<void(Response)> cb) {
    ...
    if (req.recurse) {
        ...
        distribute_command(mn, res, RPC::names()[0], req);
        lock.lock();
    }
...
void RequestHandler::process_client_req(
    ...
    if (ttl < TTL_MINIMUM || ttl > TTL_MAXIMUM) {
        BELDEX_LOG(warn, "Forbidden. Invalid TTL: {}ms", ttl.count());
    ...
    if (req.timestamp > now + STORE_TOLERANCE || req.expiry < now - STORE_TOLERANCE) {
        BELDEX_LOG(debug, "Forbidden. Invalid Timestamp: {}", to_epoch_ms(req.timestamp));
        return cb(Response{http::NOT_ACCEPTABLE, "Timestamp error: check your clock"sv});
    }

    bool entry_router = req.recurse == true;
    if (!master_node_.hf_at_least(HARDFORK_RECURSIVE_STORE))
        // "store" exists before 18.1, so don't forward requests since non-upgraded nodes can't
        // handle mn.storage_cc at all.
        req.recurse = false;

    auto [res, lock] = setup_recursive_request(master_node_, req, std::move(cb));
    auto& mine = req.recurse
        ? res->result["swarm"][master_node_.own_address().pubkey_ed25519.hex()]
        : res->result;

    bool use_old_hash = !master_node_.hf_at_least(HARDFORK_HASH_BLAKE2B);
    ...
    bool success = false;
    try {
        success = master_node_.process_store(message{
            req.pubkey, message_hash, req.timestamp, req.expiry, std::move(req.data)}, &new_msg);
    } catch (const std::exception& e) {
        BELDEX_LOG(err, "Internal Server Error. Could not store message for {}: {}",
...
void RequestHandler::process_client_req(
        rpc::beldexd_request&& req, std::function<void(beldex::Response)> cb) {

    std::optional<std::string> beldexd_params;
    if (req.params)
        ...
        [cb = std::move(cb)](bool success, auto&& data) {
            ...
            if (success && data.size() >= 2 && data[0] == "200") {
                ...
                if (result.is_discarded()) {
                    BELDEX_LOG(warn, "Invalid beldexd response to client request: result is not valid json");
            ...
            return cb({http::BAD_REQUEST,
                data.size() >= 2 && !data[1].empty()
                    ? std::move(data[1]) : "Unknown beldexd error"s});
        },
        beldexd_params);
}

void RequestHandler::process_client_req(
        rpc::get_swarm&& req, std::function<void(beldex::Response)> cb) {

    const auto swarm = master_node_.get_swarm(req.pubkey);

    BELDEX_LOG(debug, "get swarm for {}, swarm size: {}",
            obfuscate_pubkey(req.pubkey), swarm.mnodes.size());

    auto body = swarm_to_json(swarm);

    if (BELDEX_LOG_ENABLED(trace))
        BELDEX_LOG(trace, "swarm details for pk {}: {}", obfuscate_pubkey(req.pubkey), body.dump());

    cb(Response{http::OK, std::move(body)});
}

void RequestHandler::process_client_req(
        rpc::retrieve&& req, std::function<void(beldex::Response)> cb) {
    ...
    if (req.check_signature) {
        if (req.timestamp < now - SIGNATURE_TOLERANCE || req.timestamp > now + SIGNATURE_TOLERANCE) {
            BELDEX_LOG(debug, "retrieve: invalid timestamp ({}s from now)", duration_cast<seconds>(req.timestamp - now).count());
        ...
        if (!verify_signature(req.pubkey, req.pubkey_ed25519, req.signature, "retrieve", req.timestamp)) {
            BELDEX_LOG(debug, "retrieve: signature verification failed");
    ...
    try {
        msgs = master_node_.retrieve(req.pubkey, req.last_hash.value_or(""));
    } catch (const std::exception& e) {
    ...
    BELDEX_LOG(trace, "Retrieved {} messages for {}", msgs.size(), obfuscate_pubkey(req.pubkey));
...
void RequestHandler::process_client_req(
        rpc::info&&, std::function<void(beldex::Response)> cb) {
...
void RequestHandler::process_client_req(
        rpc::delete_all&& req, std::function<void(beldex::Response)> cb) {
    BELDEX_LOG(debug, "processing delete_all {} request", req.recurse ? "direct" : "forwarded");
    ...
    if (req.timestamp < now - tolerance || req.timestamp > now + tolerance) {
        BELDEX_LOG(debug, "delete_all: invalid timestamp ({}s from now)", duration_cast<seconds>(req.timestamp - now).count());
    ...
    if (!verify_signature(req.pubkey, req.pubkey_ed25519, req.signature, "delete_all", req.timestamp)) {
        BELDEX_LOG(debug, "delete_all: signature verification failed");
    ...
    if (auto deleted = master_node_.delete_all_messages(req.pubkey)) {
...
void RequestHandler::process_client_req(
    ...
    BELDEX_LOG(debug, "processing delete_msgs {} request", req.recurse ? "direct" : "forwarded");

    if (!master_node_.is_pubkey_for_us(req.pubkey))
        return cb(handle_wrong_swarm(req.pubkey));

    if (!verify_signature(req.pubkey, req.pubkey_ed25519, req.signature, "delete", req.messages)) {
        BELDEX_LOG(debug, "delete_msgs: signature verification failed");
    ...
    if (auto deleted = master_node_.delete_messages(req.pubkey, req.messages)) {
        std::sort(deleted->begin(), deleted->end());
...
void RequestHandler::process_client_req(
    ...
    BELDEX_LOG(debug, "processing delete_before {} request", req.recurse ? "direct" : "forwarded");
    ...
    if (req.before > now + 1min) {
        BELDEX_LOG(debug, "delete_before: invalid timestamp ({}s from now)", duration_cast<seconds>(req.before - now).count());
    ...
    if (!verify_signature(req.pubkey, req.pubkey_ed25519, req.signature, "delete_before", req.before)) {
        BELDEX_LOG(debug, "delete_before: signature verification failed");
    ...
    if (auto deleted = master_node_.delete_messages_before(req.pubkey, req.before)) {
        std::sort(deleted->begin(), deleted->end());
...
void RequestHandler::process_client_req(
    ...
    BELDEX_LOG(debug, "processing expire_all {} request", req.recurse ? "direct" : "forwarded");
    ...
    if (req.expiry < now - (req.recurse ? SIGNATURE_TOLERANCE : SIGNATURE_TOLERANCE_FORWARDED)) {
        BELDEX_LOG(debug, "expire_all: invalid timestamp ({}s ago)", duration_cast<seconds>(now - req.expiry).count());
    ...
    if (!verify_signature(req.pubkey, req.pubkey_ed25519, req.signature, "expire_all", req.expiry)) {
        BELDEX_LOG(debug, "expire_all: signature verification failed");
    ...
    if (auto updated = master_node_.update_all_expiries(req.pubkey, req.expiry)) {
...
void RequestHandler::process_client_req(
    ...
    BELDEX_LOG(debug, "processing expire_msgs {} request", req.recurse ? "direct" : "forwarded");
    ...
    if (!verify_signature(req.pubkey, req.pubkey_ed25519, req.signature, "expire", req.expiry, req.messages)) {
        BELDEX_LOG(debug, "expire_msgs: signature verification failed");
    ...
    if (auto updated = master_node_.update_messages_expiry(req.pubkey, req.messages, req.expiry)) {
        std::sort(updated->begin(), updated->end());
...
void RequestHandler::process_client_req(
    ...
    BELDEX_LOG(trace, "process_client_req str <{}>", req_json);

    json body = json::parse(req_json, nullptr, false);
    if (body.is_discarded()) {
        BELDEX_LOG(debug, "Bad client request: invalid json");
        return cb(Response{http::BAD_REQUEST, "invalid json"sv});
    }

    if (BELDEX_LOG_ENABLED(trace))
        BELDEX_LOG(trace, "process_client_req json <{}>", body.dump(2));

    const auto method_it = body.find("method");
    if (method_it == body.end() || !method_it->is_string()) {
        BELDEX_LOG(debug, "Bad client request: no method field");
    ...
    BELDEX_LOG(trace, "  - method name: {}", method_name);

    auto params_it = body.find("params");
    if (params_it == body.end() || !params_it->is_object()) {
        BELDEX_LOG(debug, "Bad client request: no params field");
...
void RequestHandler::process_client_req(
    ...
    if (auto it = client_rpc_endpoints.find(method_name);
        ...
        BELDEX_LOG(debug, "Process client request: {}", method_name);
        try {
    ...
    BELDEX_LOG(debug, "Bad client request: unknown method '{}'", method_name);
...
Response RequestHandler::process_retrieve_all() {
    ...
    std::vector<message> msgs;
    try {
        msgs = master_node_.get_all_messages();
    } catch (const std::exception& e) {
...
void RequestHandler::process_storage_test_req(
    ...
    auto [status, answer] = master_node_.process_storage_test_req(
    ...
    if (status == MessageTestStatus::RETRY) {
        ...
        master_node_.bmq_server()->add_timer(timer_ref, [
            ...
            BELDEX_LOG(trace, "Performing storage test retry, {} since started",
...
void RequestHandler::process_onion_req(std::string_view ciphertext,
    ...
    master_node_.record_onion_request();
...
void RequestHandler::process_onion_req(RelayToNodeInfo&& info,
    ...
    auto dest_node = master_node_.find_node(dest);
    if (!dest_node) {
    ...
    auto on_response = [cb=std::move(data.cb)](bool success, std::vector<std::string> data) {
        ...
        if (!success) {
            BELDEX_LOG(debug, "[Onion request] Request time out");
        ...
        if (data.size() < 2) {
            BELDEX_LOG(debug, "[Onion request] Invalid response; expected at least 2 parts");
            return cb({http::INTERNAL_SERVER_ERROR, "Invalid response from mnode"s});
        }
        ...
        if (res.status != http::OK)
            BELDEX_LOG(debug, "Onion request relay failed with: {}",
                    std::holds_alternative<nlohmann::json>(res.body) ? "<json>" : view_body(res));

        cb(std::move(res));
    };

    BELDEX_LOG(debug, "send_onion_to_mn, mn: {}", dest_node->pubkey_legacy);

    data.ephem_key = ekey;
    data.enc_type = etype;
    master_node_.send_onion_to_mn(
...
void RequestHandler::process_onion_req(
    ...
    BELDEX_LOG(debug, "We are to forward the request to url: {}{}",
