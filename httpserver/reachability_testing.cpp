...
static void check_incoming_tests_impl(
    ...
    if (whine) {
        ...
        if (!failing) {
            BELDEX_LOG(info, "{} ping received; port is likely reachable again", name);
        } else {
            if (incoming.last_test.time_since_epoch() == 0s) {
                BELDEX_LOG(warn, "Have NEVER received {} pings!", name);
            } else {
                BELDEX_LOG(warn,
                    "Have not received {} pings for a long time ({:.1f} mins)!",
                    name, fminutes{elapsed}.count());
            }
            BELDEX_LOG(warn, "Please check your {} port. Not being reachable "
...
void reachability_testing::check_incoming_tests(const clock::time_point& now) {
    ...
    check_incoming_tests_impl("BMQ", now, startup, last_bmq);
}

void reachability_testing::incoming_ping(ReachType type, const clock::time_point& now) {
    (type == ReachType::BMQ ? last_bmq : last_https).last_test = now;
}
