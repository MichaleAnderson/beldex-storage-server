...
static void load_pk_signature(
    ...
    if (pk_ed) {
        ...
        if (pk_ed->size() == 64) {
            ...
            bmq::from_hex(pk_ed->begin(), pk_ed->end(), rpc.pubkey_ed25519.emplace().begin());
    ...
    if constexpr (std::is_same_v<json, Dict>) {
        if (!bmq::is_base64(*sig) || !(sig->size() == 88 || (sig->size() == 86 && sig->substr(84) == "==")))
            throw parse_error{"invalid signature: expected base64 encoded Ed25519 signature"};
        bmq::from_base64(sig->begin(), sig->end(), rpc.signature.begin());
    } else {
        if (sig->size() != 64)
...
static void load(beldexd_request& o, Dict& d) {
    ...
    o.endpoint = *endpoint;
    if (!allowed_beldexd_endpoints.count(o.endpoint))
        throw parse_error{fmt::format("Invalid beldexd endpoint '{}'", o.endpoint)};
    ...
    if constexpr (std::is_same_v<Dict, json>) {
        ...
        if (auto json_str = parse_field<std::string_view>(d, "params")) {
            ...
            if (params.is_discarded())
                throw parse_error{"beldexd_request params field does not contain valid json"};
            if (!params.is_null())
                o.params = std::move(params);
        }
    }
}
void beldexd_request::load_from(json params) { load(*this, params); }
void beldexd_request::load_from(bt_dict_consumer params) { load(*this, params); }
