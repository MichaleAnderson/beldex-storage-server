...
class MasterNode {
    ...
    void send_storage_test_req(const mn_record& testee, uint64_t test_height,
                               const message& msg);

    void process_storage_test_response(const mn_record& testee,
                                       const message& msg,
    ...
    // Reports node reachability result to beldexd and, if a failure, queues the node for retesting.
  ...
  public:
    ...
    // Return true if the master node is ready to handle requests, which means the storage server
    // is fully initialized (and not trying to shut down), the master node is active and assigned
    // to a swarm and is not syncing.
    //
    // Teturns false and (if `reason` is non-nullptr) sets a reason string during initialization and
    // while shutting down.
    //
    // If this MasterNode was created with force_start enabled then this function always returns
    ...
    std::vector<mn_record> get_swarm_peers();
    ...
    // Called once we have established the initial connection to our local beldexd to set up initial
    // data and timers that rely on an beldexd connection.  This blocks until we get an initial
    // master node block update back from beldexd.
