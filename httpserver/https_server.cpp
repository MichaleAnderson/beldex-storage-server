            ...
            for (const auto& [addr, port, required] : bind)
                https.listen(addr, port, LIBUS_LISTEN_EXCLUSIVE_PORT,
                        ...
                        (us_listen_socket_t* sock) {
                            if (sock) {
                                BELDEX_LOG(info, "HTTPS server listening at {}", addr);
                                listening.push_back(sock);
                            } else if (req) {
                                required_bind_failed = true;
                                BELDEX_LOG(critical, "HTTPS server failed to bind to required address {}", addr);
                            } else {
                                BELDEX_LOG(warn, "HTTPS server failed to bind to (non-required) address {}", addr);
                            }
                        });
...
bool HTTPSServer::check_ready(HttpResponse& res) {
    if (std::string reason; !master_node_.mnode_ready(&reason)) {
        BELDEX_LOG(debug, "Storage server not ready ({}), replying with 503", reason);
        error_response(res, http::SERVICE_UNAVAILABLE, "Master node is not ready: " + reason + "\n");
        return false;
    }
    return true;
}
...
namespace {
    ...
    x25519_pubkey extract_x25519_from_hex(std::string_view hex) {
        try {
            ...
            BELDEX_LOG(warn, "Failed to decode ephemeral key in onion request: {}", e.what());
            throw;
        }
    }
    ...
    static void handle_request(
        ...
        if (auto len = req.getHeader("content-length"); !len.empty()) {
            if (uint64_t length; !util::parse_int(len, length)) {
                BELDEX_LOG(warn, "Received HTTPS request from {} with invalid Content-Length, dropping",
                ...
                BELDEX_LOG(warn, "Received HTTPS request from {} with too-large body ({} > {}), dropping",
        ...
        BELDEX_LOG(debug, "Received {} {} request from {}", req.getMethod(), request.uri, request.remote_addr);
...
void HTTPSServer::create_endpoints(uWS::SSLApp& https)
{
    ...
    https.post("/swarms/ping_test/v1", [this](HttpResponse* res, HttpRequest* req) {
        BELDEX_LOG(trace, "Received (old) https ping_test");
        master_node_.update_last_ping(ReachType::HTTPS);
        Response resp{http::OK};
        resp.headers.emplace_back(http::MNODE_SIGNATURE_HEADER, cert_signature_);
    ...
    https.post("/quit", [this](HttpResponse* res, HttpRequest* req) {
        BELDEX_LOG(info, "POST /quit");
        res.cork([this, &res] {
            res.writeStatus("200 OK");
            res.end();
            master_node_.shutdown();
        });
    });
    ...
    https.any("/*", [this](HttpResponse* res, HttpRequest* req) {
        BELDEX_LOG(info, "Invalid HTTP request for {} {} from {}",
...
/// Verifies mnode pubkey and signature values in a request; returns the sender pubkey on
...
/// mnode) but don't actually verify the signature against the body (note that this is *not*
...
static std::variant<legacy_pubkey, Response> validate_mnode_signature(MasterNode& mn, const Request& r, bool prevalidate = false) {
    legacy_pubkey pubkey;
    if (auto it = r.headers.find(http::MNODE_SENDER_HEADER); it != r.headers.end())
        pubkey = parse_legacy_pubkey(it->second);
    if (!pubkey) {
        BELDEX_LOG(debug, "Missing or invalid pubkey header for request");
        return Response{http::BAD_REQUEST, "missing/invalid pubkey header"sv};
    }
    signature sig;
    if (auto it = r.headers.find(http::MNODE_SIGNATURE_HEADER); it != r.headers.end()) {
        try { sig = signature::from_base64(it->second); }
        catch (...) {
            BELDEX_LOG(warn, "invalid signature (not b64) found in header from {}", pubkey);
            return Response{http::BAD_REQUEST, "Invalid signature"sv};
        }
    } else {
        BELDEX_LOG(debug, "Missing required signature header for request");
        return Response{http::BAD_REQUEST, "missing signature header"sv};
    }

    if (!mn.find_node(pubkey)) {
        BELDEX_LOG(debug, "Rejecting signature from unknown master node: {}", pubkey);
        return Response{http::UNAUTHORIZED, "Unknown master node"sv};
    }

    if (!prevalidate) {
        if (!check_signature(sig, hash_data(r.body), pubkey)) {
            BELDEX_LOG(debug, "mnode signature verification failed for pubkey {}", pubkey);
            return Response{http::UNAUTHORIZED, "mnode signature verification failed"sv};
        }
    }
    return pubkey;
}
...
void HTTPSServer::process_storage_test_req(HttpRequest& req, HttpResponse& res) {
    ...
    auto check_mnode_headers = [this, &res](call_data& data) {
        // Before we read the body make sure we have the required headers (so that we can reject bad
        // requests earlier).
        if (auto prevalidate = validate_mnode_signature(master_node_, data.request, true);
            ...
            if (rate_limiter_.should_rate_limit(std::get<legacy_pubkey>(prevalidate))) {
                queue_response_internal(*this, res,
                        Response{http::TOO_MANY_REQUESTS, "too many requests from this mnode"sv});
                data.replied = true;
            }
        }
    };
    ...
    handle_request(*this, bmq_, req, res, [this](std::shared_ptr<call_data> data) mutable {
        ...
        bmq.inject_task("https", "https:" + request.uri, request.remote_addr,
            ...
            if (auto it = req.headers.find(http::MNODE_SENDER_HEADER); it != req.headers.end()) {
                if (tester_pk = parse_legacy_pubkey(it->second); !tester_pk) {
                    BELDEX_LOG(debug, "Invalid test request: invalid pubkey");
                    resp.body = "invalid tester pubkey header"sv;
                    return queue_response(std::move(data), std::move(resp));
                }
            } else {
                BELDEX_LOG(debug, "Invalid test request: missing pubkey");
            ...
            if (body.is_discarded()) {
                BELDEX_LOG(debug, "Bad mnode test request: invalid json");
            ...
            try {
                ...
                resp.body = "Bad mnode test request: missing fields in json"sv;
                BELDEX_LOG(debug, std::get<std::string_view>(resp.body));
                return queue_response(std::move(data), std::move(resp));
            }
                ...
                (MessageTestStatus status, std::string answer, std::chrono::steady_clock::duration elapsed) mutable {
                    ...
                    switch (status) {
                        case MessageTestStatus::SUCCESS:
                            BELDEX_LOG(debug, "Storage test success after {}",
...
void HTTPSServer::process_storage_rpc_req(HttpRequest& req, HttpResponse& res) {
    ...
    if (addr.size() != 4) {
        ...
        BELDEX_LOG(warn, "incoming client request is not IPv4; dropping it");
        return error_response(res, http::BAD_REQUEST);
    }
    if (should_rate_limit_client(addr)) {
        BELDEX_LOG(debug, "Rate limiting client request from {}", get_remote_address(res));
        return error_response(res, http::TOO_MANY_REQUESTS);
    }
    if (!req.getHeader("x-beldex-long-poll").empty()) {
        ...
        bmq.inject_task("https", "https:" + request.uri, request.remote_addr,
            ...
            try {
                request_handler_.process_client_req(data->request.body,
                    ...
                    BELDEX_LOG(debug, "Responding to a client request after {}",
        ...
        bmq.inject_task("https", "https:" + request.uri, request.remote_addr,
            ...
            OnionRequestMetadata onion{
                ...
                [data, started](Response res) {
                    BELDEX_LOG(debug, "Got an onion response ({} {}) as edge node (after {})",
...
void HTTPSServer::shutdown(bool join)
{
    ...
    if (!sent_shutdown_)
    {
        BELDEX_LOG(trace, "initiating shutdown");
        if (!sent_startup_)
        {
        ...
        {
            loop_defer([this] {
                BELDEX_LOG(trace, "closing {} listening sockets", listen_socks_.size());
    ...
    BELDEX_LOG(trace, "joining https server thread");
    if (join)
        server_thread_.join();
    BELDEX_LOG(trace, "done shutdown");
}
