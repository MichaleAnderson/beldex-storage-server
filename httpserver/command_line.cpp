...
void command_line_parser::parse_args(int argc, char* argv[]) {
    ...
    options_.beldexd_bmq_rpc = "ipc://" + beldex_sock.u8string();
    ...
    desc_.add_options()
        ...
        ("beldexd-rpc", po::value(&options_.beldexd_bmq_rpc), "BMQ RPC address on which beldexd is available; typically ipc:///path/to/beldexd.sock or tcp://localhost:22025")
        ("bmq-port", po::value(&options_.bmq_port), "Public port to listen on for BMQ connections")
        ...
        ("stats-access-key", po::value(&options_.stats_access_keys)->multitoken(), "A public key (x25519) that will be given access to the `get_stats` bmq endpoint")
    ...
    hidden.add_options()
        ...
        ("lmq-port", po::value(&options_.bmq_port), "Backwards compatible old name for --bmq-port")
        ;
    // clang-format on
    ...
    if (!vm.count("beldexd-rpc") && (!old_rpc_ip.empty() || old_rpc_port != 0)) {
        ...
        throw std::runtime_error{
            ...
            "with the BMQ RPC address instead"
        };
    }

    if (!vm.count("bmq-port") && !vm.count("lmq-port")) {
        throw std::runtime_error(
            "bmq-port command line option is not specified");
    }
