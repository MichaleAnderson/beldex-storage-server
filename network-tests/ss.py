...
def get_swarm(bmq, conn, sk, netid=5):
    pubkey = "{:02x}".format(netid) + (sk.verify_key if isinstance(sk, SigningKey) else sk.public_key).encode().hex()
    r = bmq.request(conn, "storage.get_swarm", [json.dumps({"pubkey": pubkey}).encode()])
    assert(len(r) == 1)
...
def store_n(bmq, conn, sk, basemsg, n, offset=0, netid=5):
    msgs = []
    ...
    for i in range(n):
        ...
        msgs[-1]['future'] = bmq.request_future(conn, "storage.store", [json.dumps(msgs[-1]['req']).encode()])
